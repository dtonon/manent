...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "manent");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "manent");
  }

  gtk_window_set_default_size(window, 1280, 720);
  gtk_widget_show(GTK_WIDGET(window));

  // Set window icon from AppImage bundle or local assets
  {
    const gchar* appdir = g_getenv("APPDIR");
    g_autofree gchar* icon_path = nullptr;
    if (appdir != nullptr) {
      icon_path = g_build_filename(appdir, "manent.png", nullptr);
    } else {
      g_autofree gchar* exe_link = g_file_read_link("/proc/self/exe", nullptr);
      if (exe_link != nullptr) {
        g_autofree gchar* exe_dir = g_path_get_dirname(exe_link);
        icon_path = g_build_filename(exe_dir, "data", "flutter_assets", "assets", "icon.png", nullptr);
      }
    }
